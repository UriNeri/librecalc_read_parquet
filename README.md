...
The macro is in read_parquet.py

Test input included in the repo root:
sequences_rdrp_motif_results.parquet

Before running it, edit `external_env_path` in read_parquet.py so it points to the `site-packages` directory of a Python installation that has `polars` installed.

Install and run:

1. clone into something like `/home/YOU/.config/libreoffice/4/user/Scripts/python/read_parquet.py`
